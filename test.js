...
test('SVG to Object Test', function (t) {
    ...
    // SVG File
    t.equal(typeof svgson('test.svg'), 'object', 'SVG file converted to Object');
    t.equal(typeof svgson('test.svg', { json: true }), 'string', 'SVG file converted to JSON');

    // Multiple files
    t.equal(svgson(['test.svg', './svg/at.svg']).length, 2, 'Get multiple files from Array');
    t.equal(JSON.stringify(svgson(['test.svg', '/svg/at.svg'], { title: true })[0]), JSON.stringify(svgson('test.svg', { title: true })), 'Same output from same file with single and multiple input');

    // Directory
    t.equal(typeof svgson('svg'), 'object', 'SVG files in directory converted to Object');
    t.equal(typeof svgson('svg', { json: true }), 'string', 'SVG files in directory converted to JSON');
    t.equal(svgson('svg') instanceof Array, true, 'Directory results in an Array');
    ...
    // SVGO
    t.notEqual(
      svgson('test.svg', { json: true, svgo: false }),
      svgson('test.svg', { json: true, svgo: true }),
      'Output change with svgo active'
    );
    t.notEqual(
      svgson('svg', { json: true, svgo: false }),
      svgson('svg', { json: true, svgo: true }),
      'Output in more files change with svgo active'
    );

    // Title
    t.equal(svgson('test.svg', { svgo: true, title: true }).title, 'test', 'Object title same as File name')
    ...
    // customAttrs
    t.equal(svgson('test.svg', { svgo: true, title: true, customAttrs: { hello: 'world' } }).hello, 'world', 'Assigned Custom Attributes')
