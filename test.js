...
var a = svg2obj(['test.svg', '/svg/at.svg'], { title: true })[0];
var b = svg2obj('test.svg', { title: true });

console.log( a.attrs == b.attrs )
...
test('SVG to Object Test', function (t) {
    ...
    // Multiple files
    t.equal(svg2obj(['test.svg', './svg/at.svg']).length, 2, 'Get multiple files from Array');
    t.equal(JSON.stringify(svg2obj(['test.svg', '/svg/at.svg'], { title: true })[0]), JSON.stringify(svg2obj('test.svg', { title: true })), 'Same output from same file with single and multiple input');
